...
const METRICS_PUBLISH_INTERVAL_SECS: u64 = 5;
const DEVICE_PUBLISH_INTERVAL_SECS: u64 = 10; // Less frequent for devices
const FLOW_TIMEOUT_SECS: u64 = 60 * 5; // 5 minutes for flow timeout

const NETWORK_METRICS_CHANNEL: &str = "network_metrics_channel";
...
struct PublishedFlowKey {
    ...
    src_port: u16,
    dst_port: u16,
    protocol: String, // String representation of protocol
}
...
struct PublishedDiscoveredDevice {
    ...
    mac_addr: String,
    last_seen: u64,
    timestamp: u64, // Publish time
}

// Original structs for internal aggregation
...
type MetricCache = Arc<Mutex<InterfaceAggregatedMetrics>>;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let interface_name = env::args().nth(1).unwrap_or_else(|| {
        ...
        // Attempt to find a default interface
    ...
    let interface_name_clone = interface.name.clone(); // For publisher task

    // Periodic Metric Publisher Task
    let metric_cache_publisher_clone = metric_cache.clone();
    let if_name_for_metrics = interface_name_clone.clone();
    tokio::spawn(async move {
        let mut interval = tokio::time::interval(Duration::from_secs(METRICS_PUBLISH_INTERVAL_SECS));
        loop {
            interval.tick().await;
            ...
            // Reset counters after preparing for publish (or publish deltas)
            // For simplicity here, we publish totals and then they'd be diffed by consumer or reset.
            // A more robust approach might be to calculate deltas or have Python side manage state.
            // For now, let's just log the size and imagine publishing.
            // metrics_guard.packets_in = 0; // Example of resetting if publishing deltas
            // metrics_guard.bytes_in = 0;
            // metrics_guard.packets_out = 0;
            // metrics_guard.bytes_out = 0;
                ...
                match packet_result {
                    Ok(packet_data) => {
                        if let Some(ethernet_packet) = EthernetPacket::new(packet_data) {
                             handle_ethernet_packet(&interface, &ethernet_packet, device_cache.clone(), metric_cache.clone());
                        } else {
                    ...
                    Err(e) => {
                        ...
                        // Potentially break or attempt to re-establish channel
                    }
                }
            }
...
fn handle_ethernet_packet(
    ...
    let now = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    let packet_length = ethernet.packet().len() as u64;
    let interface_mac = interface.mac.unwrap_or(MacAddr::zero());

    let mut metrics = metric_cache.lock().unwrap();

    // Determine packet direction
    let is_outgoing = ethernet.get_source() == interface_mac;
    let is_incoming = ethernet.get_destination() == interface_mac || ethernet.get_destination().is_broadcast() || ethernet.get_destination().is_multicast();

    if is_incoming {
        metrics.packets_in += 1;
        metrics.bytes_in += packet_length;
    }
    if is_outgoing { // Can be both if it's to self, but less common for typical traffic
        metrics.packets_out += 1;
        metrics.bytes_out += packet_length;
    }
            ...
            if let Some(ipv4_packet) = Ipv4Packet::new(ethernet.payload()) {
                ...
                let (src_port, dst_port) = match protocol {
                    IpNextHeaderProtocols::Tcp => {
                        TcpPacket::new(ipv4_packet.payload()).map_or((0,0), |p| (p.get_source(), p.get_destination()))
                    }
                    IpNextHeaderProtocols::Udp => {
                        UdpPacket::new(ipv4_packet.payload()).map_or((0,0), |p| (p.get_source(), p.get_destination()))
                    }
                    _ => (0,0) // No ports for ICMP or other protocols
                };

                if src_port != 0 && dst_port != 0 { // Only track flows with ports
                    let flow_key = FlowKey { src_ip, dst_ip, src_port, dst_port, protocol };
                    let flow_entry = metrics.active_flows.entry(flow_key).or_insert_with(|| FlowData {
                        packet_count: 0,
                        byte_count: 0,
                        first_seen: now,
                        last_seen: now,
                    ...
                    flow_entry.byte_count += packet_length;
                    flow_entry.last_seen = now;
                }
                 log::trace!(
                    "[{}] IPv4 | {} -> {} | Proto: {:?} | Len: {} bytes",
                    interface.name, src_ip, dst_ip, protocol, packet_length
                );
            }
        }
        EtherTypes::Arp => {
            if let Some(arp_packet) = ArpPacket::new(ethernet.payload()) {
                ...
                if sender_mac != MacAddr::zero() && sender_ip != Ipv4Addr::new(0,0,0,0) {
                    ...
                    if let Some(existing_device) = cache.get(&sender_mac) {
                        // Only publish if it's been a while since the last update for this device
                        if current_time.saturating_sub(existing_device.last_seen) < DEVICE_PUBLISH_INTERVAL_SECS {
                            publish_update = false;
                        }
                    }
                    
                    cache.insert(sender_mac, device_info.clone()); // Update cache regardless
        ...
        EtherTypes::Ipv6 => {
            if let Some(ipv6_packet) = Ipv6Packet::new(ethernet.payload()) {
                ...
                let dst_ip = IpAddr::V6(ipv6_packet.get_destination());
                 // packet_length already defined above

                let (src_port, dst_port) = match protocol {
                    IpNextHeaderProtocols::Tcp => {
                        TcpPacket::new(ipv6_packet.payload()).map_or((0,0), |p| (p.get_source(), p.get_destination()))
                    }
                    IpNextHeaderProtocols::Udp => {
